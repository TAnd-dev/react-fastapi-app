...
from app.favorite.schemas import SFavorite, SAddFavorite
...
async def is_item_in_card(item_id: int, user: Optional[Users] = Depends(current_user)) -> bool:
    return bool(await FavoriteService.find_item_model(user.id, item_id))


@router.post('/add_item')
async def add_item_to_favorite(favorite: SAddFavorite, user: Optional[Users] = Depends(current_user)):
    await FavoriteService.add_item(related_id=user.id, item_id=favorite.item_id)


@router.delete('/remove_item')
async def remove_item_from_favorite(favorite: SAddFavorite, user: Optional[Users] = Depends(current_user)):
