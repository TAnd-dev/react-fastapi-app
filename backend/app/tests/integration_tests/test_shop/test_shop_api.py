...
@pytest.mark.parametrize('search_text, count', [
    ('Test Item 1', 1),
    ('Test Item', 2),
    ('Non-existent', 0),
])
...
@pytest.mark.parametrize('client, rate, status_code', [
    ('auth_cl', 5, 200),
    ('auth_cl', 6, 422),
    ('auth_cl', 0, 422),
    ('cl', 5, 401),
])
async def test_add_review(client, rate, status_code, ac: AsyncClient, auth_ac: AsyncClient):
    async_client = ac if client == 'cl' else auth_ac
    response = await async_client.post('/shop/item/1/add_review', json={
        'text': 'Added review with test',
        'rate': rate
    })
