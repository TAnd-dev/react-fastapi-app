...
@pytest.mark.parametrize('email, password1, password2, status_code, user_id', [
...
async def test_register_user(email, password1, password2, status_code, user_id, ac: AsyncClient):
    response = await ac.post('/user/auth/register', json={
        'email': email,
        'password1': password1,
        'password2': password2,
    })
...
async def test_login_user(email, password, status_code, ac: AsyncClient):
    response = await ac.post('user/auth/login', json={
        'email': email,
        'password': password,
    })
...
@pytest.mark.parametrize('name, second_name, number_phone', [
...
async def test_change_user_profile(name, second_name, number_phone, auth_ac: AsyncClient):
