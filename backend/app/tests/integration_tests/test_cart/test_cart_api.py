...
@pytest.mark.parametrize('item_id, is_item_in_cart', [
    (1, True),
    (3, False),
])
...
async def test_add_item_to_cart(item_id, auth_ac: AsyncClient):
    response = await auth_ac.post('/cart/add_item', json={
        'item_id': item_id,
        'count': 1
    })
    assert response.status_code == 200
...
@pytest.mark.parametrize('item_id, count, is_exists', [
    (1, 10, True),
    (2, 10, True),
    (345, 10, False),
])
async def test_set_item_count_cart(item_id, count, is_exists, auth_ac: AsyncClient):
    response = await auth_ac.patch('/cart/set_count', json={
        'item_id': item_id,
        'count': count
    })
    assert response.status_code == 200
