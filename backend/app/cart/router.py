...
from app.cart.schemas import SAddChangeCart, SCart, SDeleteCart
...
async def add_item_to_cart(
    cart: SAddChangeCart, user: Optional[Users] = Depends(current_user)
):
    await CartService.add_item(
        related_id=user.id, item_id=cart.item_id, count=cart.count
    )


@router.patch('/set_count')
async def set_item_count_cart(
    cart: SAddChangeCart, user: Optional[Users] = Depends(current_user)
):
...
async def remove_item_from_cart(
    cart: SDeleteCart, user: Optional[Users] = Depends(current_user)
):
